...
(function () {
  ...
  document.getElementById('overlaps').innerHTML =
    (D.set_level.overlaps || []).map(o => `<li>${o.a_id} ~ ${o.b_id} <span class="muted">(${o.score})</span></li>`).join('') || '<li class="muted">none</li>';

  const rows = [...D.requirements].sort((a, b) => (a.overall || 9) - (b.overall || 9));
  const badge = s => s == null ? '<span class="muted">–</span>'
    : `<span class="badge" style="background:${scoreColor(s)}">${s}</span>`;
  document.getElementById('reqrows').innerHTML = rows.map((r, i) => `
    <tr class="req" data-i="${i}"><td>${r.req_id}</td>
      <td>${r.text.slice(0, 70)}${r.text.length > 70 ? '…' : ''}</td>
